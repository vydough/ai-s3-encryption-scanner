# AI-Based (Gemini) S3 Encryption Scanner (With Daily Scheduler Automation)
...
## Additional Implementations (update)
### Email Notifications (SNS)
This feature emails/notifies users when the script is activated. 

1. **Creating SNS Email Alert Topic**: I created AWS SNS Topic, creating subscription for my Email endpoint, keeping the ARN as an SNS_TOPIC_ARN environment variable in the lambda function. 
2. **Adding IAM Permissions**: I added an IAM policy, creating a custom 'SNSPublishPolicy' to the current role used by the function 'LambdaS3ScannerRole'. This policy involves permissions that allow publishing.
3. **Modifying the Lambda code**: Changed the Lambda code to include SNS logic: 
'sns_client.publish()'
